...
| Código | Indicador | Puntos | Tratamiento | Nivel | Cubierto hoy por | Propuesto | Cruces | Accionable |
|---|---|---|---|---|---|---|---|---|
| TR1 | Número total de vehículos (carros y motos) dividido entre la población total del campus | 200 | decisión | 5 | `mov_parqueo` (`mision_movilidad.gd`, "Permisos de Estacionamiento" — parcial, no mide la proporción exacta) | `tr_permisos` — proyecto B (sección 7 del diseño) | — | sí |
| TR2 | Servicios de transporte interno (shuttle) | 300 | decisión | 5 | `mov_shuttle` (`mision_movilidad.gd`, "Rutas de Shuttle Interno") | `tr_shuttle` — proyecto B | — | sí |
| TR3 | Disponibilidad de Vehículos de Cero Emisiones (ZEV) en el campus | 200 | decisión | 5 | `mov_zev` (`mision_movilidad.gd`, "Vehículos de Cero Emisiones") | `tr_flota` — proyecto B | flota eléctrica → Energía (EC) | sí |
| TR4 | Número total de Vehículos de Cero Emisiones (ZEV) dividido entre la población total del campus | 200 | decisión | 5 | `mov_zev` (mismo escenario que TR3 — parcial) | `tr_flota` — proyecto B | flota eléctrica → Energía (EC) | sí |
| TR5 | Proporción del área de estacionamiento en superficie sobre el área total del campus | 200 | decisión | 5 | `mov_ciclovia` (`mision_movilidad.gd`, "Reducción de Área de Estacionamiento" — parcial) | `tr_lote` (lote poco usado → ciclovía) — proyecto B | ciclovía → Entorno (SI) | sí |
| TR6 | Programa para limitar o disminuir el área de estacionamiento en el campus en los últimos 3 años (2021-2023) | 200 | decisión | 5 | `mov_ciclovia` | `tr_lote` — proyecto B | ciclovía → Entorno (SI) | sí |
| TR7 | Número de iniciativas para disminuir los vehículos privados en el campus | 200 | decisión | 5 | `mov_dia_sin_carros`, `mov_carpool` (`mision_movilidad.gd`) | `tr_dia_sin_carros`, `tr_carpool`, `tr_bici_bloque_e`, `tr_bici_cafetin` — proyecto B (los kits ya existen en `bicicletero_bloque_e`/`bicicletero_cafetin`, `mision_bicicletero.gd`) | día sin carros → Educación (evento); bicicletero techado con panel → Energía (EC) | sí |
